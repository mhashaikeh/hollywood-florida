function updateFeeByCapId(fcode, fsched, fperiod, fqty, finvoice, pDuplicate, recCapID) {
	...
	getFeeResult = aa.finance.getFeeItemByFeeCode(recCapID, fcode, fperiod);
	...
	if (getFeeResult.getSuccess()) {
		...
		for (feeNum in feeList)
			if (feeList[feeNum].getFeeitemStatus().equals("NEW") && !feeUpdated) // update this fee item
			{
				...
				var editResult = aa.finance.editFeeItemUnit(recCapID, adjustedQty + feeList[feeNum].getFeeUnit(), feeSeq);
				feeUpdated = true;
	...
	if (!feeUpdated && adjustedQty != 0 && (!invFeeFound || invFeeFound && pDuplicate == "Y"))
		feeSeq = addFee(fcode, fsched, fperiod, adjustedQty, finvoice, recCapID);
	else
		feeSeq = null;
	updateFeeItemInvoiceFlagByCapId(feeSeq,finvoice,recCapID);
	return feeSeq;
} 
