...
if (pfResult.getSuccess())
    ...
    }				// fee item list in number type
for(xx in feeSeqListString)
{ 
    logDebug("FeeSeqListString: " + feeSeqListString[xx]);

    for (ij in pfObj){
        logDebug("pfObj: " + pfObj[ij].getFeeSeqNbr());
		if (Number(feeSeqListString[xx])== pfObj[ij].getFeeSeqNbr()){
            if(pfObj[ij].getFeeAllocation() == 0){
                continue;
            }
        }
    }
