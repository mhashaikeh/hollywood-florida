...
// JSON Configuration as provided earlier...

Notes:
// Notes as provided earlier...
...
try {
    ...
    if (isConfigurableScript(settingsArray, scriptSuffix)) {
        for (s in settingsArray) {
            ...
            // Replace placeholder in action with actual value from pre-script
            if (typeof licenseType !== 'undefined') {
                rules.action.refLPType = licenseType;
            }
...
function licenseIssuance(itemCapId, recordSettings) {
    ...
    var rApplyASIASLPNumber = handleUndefined(recordSettings.applyASIASLPNumber, false);
    ...
    if (rParentLicense != "") {
        //Case 01489938 - start
        //Avoid the duplicate license created when workflow complete two times.
        var parentList = getParents(rParentLicense);
    ...
    if (recordSettings.createLP) {
        ...
        var lpNumber = null;
        if (rApplyASIASLPNumber) {
            lpNumber = getAppSpecific(recordSettings.applyASIASLPNumber, itemCapId);
        }
        if (lpNumber) {
            createRefLP4Lookup(lpNumber, recordSettings.refLPType, recordSettings.contactType, contactAddressType);
        } else {
        ...
        var rNewLP = aa.licenseScript.getRefLicensesProfByLicNbr(aa.serviceProvider, lpNumber || rNewLicIdString).getOutput();
        if (rNewLP) {
            var rThisLP = rNewLP[0];
            ...
            if (recordSettings.expirationType == "Function" && rCustomExpirationFunction != null && rCustomExpirationFunction != "") {
                var dateCalculationFuntion = rCustomExpirationFunction;
                var dateResult = eval("(" + dateCalculationFuntion + ")");
            ...
            if (!isEmptyOrNull(lpNumber || rNewLicIdString)) {
                ...
                if (reqContact != null && reqContact["email"] != null && reqContact["email"] != "") {
                    ...
                    if (thisPublicUser != null) {
                        ...
                        if (!isLicenseConnectedToPublicUser(publicUserSeqNum, lpNumber || rNewLicIdString)) {
                            associateLPToPublicUser(lpNumber || rNewLicIdString, publicUserSeqNum);
