/*------------------------------------------------------------------------------------------------------/
| Program : ACA_SITE_IMP_SCOPE_VALIDATION.JS
| Event   : ACA_Before
|
...
try {
    ...
    if (type < 3) {
        cancel = true;
        showMessage = true;
        logMessage("At least three Scope of Work items must be selected for this record. If fewer than three are applicable, please apply for individual permits instead.");
    }
    
} catch (err) {
    var emailAddress = "jshear@mytechsinc.com"; // email to send report
    aa.sendMail("no-reply@accela.com", emailAddress, "", "ACA_SITE_IMP_SCOPE_VALIDATION", err + debug + err.stack);
}
