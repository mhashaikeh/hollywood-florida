/*------------------------------------------------------------------------------------------------------/
| Program : ACA_REQUIRED_BUILDING_DOCS.JS
| Event   : ACA_Before
|
...
try {
    ...
    if (appTypeString == "Building/Antenna/NA/NA") {
        var towerType = AInfo["Tower Type"];
        if (towerType == "Free Standing"){
            r[documentCount] = "Special Building Inspector Form";
            documentCount++;
        }
    }
    ...
    aa.sendMail("no-reply@accela.com", emailAddress, "", "ACA_REQUIRED_BUILDING_DOCS", err + debug + err.stack);
}
