/*------------------------------------------------------------------------------------------------------/
| Program : ACA_REQUIRED_ENFORCEMENT_DOCS.JS
| Event   : ACA_Before
|
...
try {
    ...
    if (appTypeString == "Enforcement/Permit/Dog/Renewal") {
        r[documentCount] = "Photo";
        documentCount++;

        // Check if Rabies Vaccination is expired
        var parentCapIdResult = cap.getParentCapID();
        if (parentCapIdResult) {
            var pca = parentCapIdResult.toString().split("-");
            parentCapId = aa.cap.getCapID(pca[0], pca[1], pca[2]).getOutput();
            if (parentCapId) {
                var vaxExp = getAppSpecific("Rabies Vaccination Expiration Date", parentCapId); 
                if (vaxExp) {
                    var vaxExpDate = new Date(vaxExp);
                    if (vaxExpDate < currentDate) {
                        r[documentCount] = "Proof of Rabies Vaccination";
                        documentCount++;
                    }
                }
            }
    ...
    aa.sendMail("no-reply@accela.com", emailAddress, "", "ACA_REQUIRED_ENFORCEMENT_DOCS", err + debug + err.stack);
}
