...
try {
    ...
    if (appMatch("Building/Fence/NA/NA")){
        ...
        var fenceHeight = AInfo['Fence Height'];

        if (matches(typeOfProperty,"Single Family","Manufactured Home")){
        	if (locOfFence == "Front"){
	        	if (fenceHeight > 4){
                    showMessage = true;
                    cancel = true;
                    comment("Fence Height Cannot Exceed 4 ft for this type of property and Location of Fence");
	        	}
	        }
	        if (matches(locOfFence,"Rear","Side")){
	        	if (fenceHeight > 6){
                    showMessage = true;
                    cancel = true;
                    comment("Fence Height Cannot Exceed 6 ft for this type of property and Location of Fence");
	        	}
	        }
        }

        if (matches(typeOfProperty,"Commercial","Multi-Family")){
	        if (matches(locOfFence,"Rear","Side")){
	        	if (fenceHeight > 8){
                    showMessage = true;
                    cancel = true;
                    comment("Fence Height Cannot Exceed 8 ft for this type of property and Location of Fence");
	        	}
	        }
	    }
        ...
        if (matches(fenceMaterial,"Chain Link","PVC")){
            ...
            var zoneTOC = getGISInfo2ASB("HOLLYWOOD", "TOC", "LANDUSEDESC");
            var zoneRAC = getGISInfo2ASB("HOLLYWOOD", "RAC", "LANDUSEDESC");
            ...
            if (fenceMaterial == "Chain Link"){
                ...
                if (typeOfProperty == "Commercial" && locOfFence == "Front"){
                    if (zoneTOC == "Transportation Oriented Corridor"){
                        showMessage = true;
                        cancel = true;
                ...
                if (recField == "No"){
                    if (!matches(zoneHist,null,undefined,"")){
                        var zoneHistString = String(zoneHist).toLowerCase();
                        if (zoneHistString.indexOf("hist") != -1){
                            showMessage = true;
                            cancel = true;
                            comment("Your location of work indicates a historic district. A chain link fence is not permitted.");
                        }
                    }
                }
             
                if (zoneRAC == "Regional Activity Center"){
                    showMessage = true;
                    cancel = true;
    ...
    aa.sendMail("no-reply@accela.com", emailAddress, "", "ACA_REQUIRE_LICENSE_PROFESSIONAL", err + debug + err.stack);
}
