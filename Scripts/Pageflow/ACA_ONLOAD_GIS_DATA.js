/*------------------------------------------------------------------------------------------------------/
| Program : ACA_ONLOAD_GIS_DATA.JS
| Event   : ACA_Before
|
...
try {
    ...
    var zoneCRA = getGISInfo2ASB("HOLLYWOOD", "CRA", "CRA_District");

    if (!matches(zoneHist,null,undefined,"")){
        editAppSpecific4ACA("Historic District Description", zoneHist);
    }
    if (!matches(zoneRAC,null,undefined,"")){
        editAppSpecific4ACA("RAC Land Use Code", zoneRAC);
    }
    if (!matches(zoneTOC,null,undefined,"")){
        editAppSpecific4ACA("TOC Land Use Code", zoneTOC);
    }
    if (!matches(zoneCRA,null,undefined,"")){
        editAppSpecific4ACA("CRA District", zoneCRA);
    }
    ...
    aa.sendMail("no-reply@accela.com", emailAddress, "", "ACA_ONLOAD_GIS_DATA", err + debug + err.stack);
}
