/*------------------------------------------------------------------------------------------------------/
| Program : ACA_ROW_DRAINAGE_VALIDATION.JS
| Event   : ACA_Before
|
...
try {
    ...
    for (var i = 0; i < requiredFields.length; i++) {
        if (AInfo[requiredFields[i]] == "CHECKED") {
            isChecked = true;
            break; 
        }
    }
    ...
    if (!isChecked) {
        showMessage = true;
        cancel = true;
        comment("Must Select at least one item within Scope of Work");
    }
} catch (err) {
    var emailAddress = "jshear@mytechsinc.com"; // Email to send report
    aa.sendMail("no-reply@accela.com", emailAddress, "", "ACA_ROW_DRAINAGE_VALIDATION", err + debug + err.stack);
}
