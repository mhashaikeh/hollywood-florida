...
var stLicNbr = expression.getValue("ASI::LICENSE INFORMATION::State License Number");
...
try {
    ...
    if (currentCapId != null && currentCapType.value != null && currentCapType.value.equals(String('Licenses/Contractor/State/Application')) && stLicNbr.value != null && stLicNbr.value != "") {
        
        var countyLicNumExists = isContractorCountyLicenseRegistered(stLicNbr.value, currentCapId);
        var stateLicNumExists = isContractorStateLicenseRegistered(stLicNbr.value, currentCapId);
    ...
    asiForm.message = "error in expression CTR_STATE_LIC_NBR_VALIDATION : " + ex;
    expression.setReturn(asiForm);
}
