/*------------------------------------------------------------------------------------------------------/
| Program		: CTRCA;~!~!~!~.js
| Event			: CTRCA:Convert To Real Cap After
| Notes			: FOR TICKETS: HLYWDFL-576
| Created by	: JSCHULER
| Created at	: 5/16/2024
/------------------------------------------------------------------------------------------------------*/
try{	
	...
	if (appTypeArray[0] != "Licenses" && appTypeArray[1] != "Amendment") {
		...
		if (licenseProfResult.getSuccess()) {
			...
			if (licenseProfList && licenseProfList.length > 0) {  // Only proceed if there are license professionals
				...
				logDebug("No license professionals found for notification.");
			}
		}
	}
	...
	if (appTypeArray[0] != "Licenses" && appTypeArray[1] != "Amendment") {
		...
		if (LPs.getSuccess()) {
			...
			if (LPsArr && LPsArr.length > 0) {  // Only proceed if there are license professionals
				...
				logDebug("No license professionals found for applying conditions.");
			}
		}
	}
    ...
    aa.sendMail("no-reply@accela.com", emailAddress, "", "CTRCA:*/*/*/*", err + debug + err.stack);
};