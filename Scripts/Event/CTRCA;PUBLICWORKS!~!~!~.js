try {
    ...
    //Start: Apply Waste Management License Condition
    if (appMatch("PublicWorks/Utilities/Onsite Drainage/NA")) {
        if (!matches(AInfo['Type of Property'], "Other", "Manufactured Home", "Single Family")) {
            if (!appHasCondition("Permit", "Applied", "Broward County Surface Water Management License Required", null)) {
                addStdCondition("Permit", "Broward County Surface Water Management License Required");
            }
        }
    }
    //End: Apply Waste Management License Condition
    ...
    if (appMatch("PublicWorks/Engineering/Right of Way/NA")){
        ...
        //Start: calculate total Linear Feet
        var rowTable = loadASITable("ROADWAY DETAILS");
        var totalLinearFt = 0;

        if (rowTable && rowTable.length > 0) {
            for (var rowIndex in rowTable) {
                var thisRow = rowTable[rowIndex];
                var lengthValue = thisRow["Length of Road (in Feet)"].fieldValue;

                if (!isNaN(lengthValue) && lengthValue != null && lengthValue !== "") {
                    totalLinearFt += parseFloat(lengthValue);
                }
            }
        }
        editAppSpecific("Total Linear Ft", totalLinearFt);
        //End: calculate total Linear Feet
    }
    ...
    //Start Apply Parent to Sub-Permit
    if (!matches(AInfo["Related Permit Number"], null, "", "undefined")) {
        addParent(AInfo["Related Permit Number"]);
    }
    //End Apply Parent to Sub-Permit
} catch (err) {
    var emailAddress = "jshear@mytechsinc.com"; //email to send report
    aa.sendMail("no-reply@accela.com", emailAddress, "", "CTRCA:PUBLICWORKS/*/*/*", err + debug + err.stack);
};
