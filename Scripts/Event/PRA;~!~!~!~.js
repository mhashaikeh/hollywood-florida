try {
	...
	if (appTypeArray[2] == "Plans Change") {
		if (balanceDue <= 0 && isTaskActive("Plans Coordination")) {
			closeTask("Plans Coordination", "Approved", "", "");
			updateAppStatus("Closed - Approved", "Updated By PRA:*/*/*/* Script");
		}
	}

} catch (err) {
	var emailAddress = "accelaerrors@hollywoodfl.org"; //email to send report
	aa.sendMail("no-reply@accela.com", emailAddress, "", "PRA:*/*/*/*", err + debug + err.stack);
}